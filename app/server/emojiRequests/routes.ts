...
import { OUTCOMES, isOutcome, type Outcome } from './outcome.js'
...
const HANDLED_FIELDS = new Set(['outcome', 'pr_url'])

function parseHandled(body: unknown):
  | { ok: true; fields: { outcome: Outcome; prUrl: string | null } }
  | { ok: false; details: Record<string, string> } {
  const details = Object.create(null) as Record<string, string>
  if (typeof body !== 'object' || body === null || Array.isArray(body)) {
    return { ok: false, details: { body: 'must be a JSON object' } }
  ...
  const record = body as Record<string, unknown>
  rejectUnknownFields(record, HANDLED_FIELDS, details)

  if (!isOutcome(record.outcome)) details.outcome = `must be one of ${OUTCOMES.join(', ')}`

  let prUrl: string | null = null
  const raw = typeof record.pr_url === 'string' ? record.pr_url.trim() : record.pr_url
  if (raw === undefined || raw === null || raw === '') {
    prUrl = null
  } else if (typeof raw !== 'string' || raw.length > 500) {
    details.pr_url = 'must be a string of at most 500 characters'
  } else {
    prUrl = raw
  }

  if (Object.keys(details).length > 0) return { ok: false, details }
  return { ok: true, fields: { outcome: record.outcome as Outcome, prUrl } }
}
...
export function emojiRequestsRouter(
  ...
  router.get('/', writeGate, async (req, res) => {
    const pending = req.query.pending === '1' || req.query.pending === 'true'
    res.json(await store.list({ pending }))
  })

  router.patch('/:id', writeGate, async (req, res) => {
    const { id } = req.params as { id: string }
    if (!UUID_RE.test(id)) {
      sendValidation(res, { id: 'must be a uuid' })
      return
    }
    const parsed = parseHandled(req.body)
    if (!parsed.ok) {
      sendValidation(res, parsed.details)
      return
    }
    const row = await store.markHandled(id, parsed.fields)
    if (row === null) {
